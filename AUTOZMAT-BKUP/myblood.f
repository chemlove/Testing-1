...
C         PRINT *,'IRES2 of non-dummy atoms', IRES2(MINDUM), MINDUM
          MINDUM = MINDUM + 1
C         IF (I .EQ. J-1) MINDUM1 =MINDUM 
          ENDIF
        ENDIF
   55 CONTINUE
      MINDUM = MINDUM - 1
C     PRINT *, 'Atoms excluding dummy atoms=' ,MINDUM
      IF (MINDUM.NE.QMNUM) WRITE (*,*) 'MINDUM AND QMNUM DO NO EQUAL'

c     IF (J.EQ.100) THEN
c     PRINT*, 'This is a single molecule.' 
c     PRINT*, 'It contains', MINDUM, 'atoms.'
c     ENDIF
c
c     IF (J.NE.100) THEN
c     PRINT*, 'This is a complex.' 
c     PRINT*, 'It contains', MINDUM1, 'atoms in the first molecule and',
c    *        MINDUM-MINDUM1, 'atoms in the second molecule.'
c     ENDIF
